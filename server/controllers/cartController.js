...
export const addToCart = async (req, res) => {
    ...
    const foundProduct =await Product.findById(id)
        ...
        const updstedCart=    await  Cart.updateOne({user: user}, {$push: {
            cartItems:productToAdd }})
 ...
 export const removeitem = async (req, res) => {
    ...
    await Cart.updateOne({user: user}, 
...
export const myCart =async (req, res)=>{
  ...
  if ( cart.length===0 ){
   res.status(400).send("empty cart")
    ...
    await Cart.updateOne({user: user}, { $set: { grandtotalPrice:totalPrice  }},   { new: true });
    const endresult= 
